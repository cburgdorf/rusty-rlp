...
fn encode_list_with_strings_and_ints() {
    let sample = vec![
        Data::Str("dog".to_string()),
        Data::Str("mouse".to_string()),
        Data::Str("tigers".to_string()),
        Data::Int(127),
    ];
    ...
    assert_eq!(
        stream.out(),
        vec![
            0xd2, 0x83, 0x64, 0x6f, 0x67, 0x85, 0x6d, 0x6f, 0x75, 0x73, 0x65, 0x86, 0x74, 0x69,
            0x67, 0x65, 0x72, 0x73, 0x7f
        ]
    );
}
...
fn encode_nested_list_with_strings() {
    ...
    assert_eq!(
        stream.out(),
        vec![
            209, 132, 102, 100, 115, 97, 203, 131, 100, 102, 115, 198, 133, 106, 102, 100, 107, 108
        ]
    );
}
